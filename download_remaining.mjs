...
const teams = {
  RCB: 'Royal_Challengers_Bengaluru_logo.png',
  DC:  'Delhi_Capitals_logo.svg',
  RR:  'Rajasthan_Royals_Logo.svg',
  SRH: 'Sunrisers_Hyderabad_logo.svg',
...
async function download() {
  for (const [team, file] of Object.entries(teams)) {
    try {
      ...
      fs.writeFileSync(path.join(dir, `${team}${ext}`), Buffer.from(buffer));
      console.log(`Saved ${team}${ext}`);
    } catch (err) {
